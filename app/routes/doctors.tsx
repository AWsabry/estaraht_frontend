...
import { Search, Trash2, Star, DollarSign, Users, Calendar, Plus, X, Eye } from 'lucide-react';
...
export default function Doctors() {
  ...
  const filteredDoctors = doctors.filter((doctor) => {
    ...
    const matchesSearch = name.includes(normalizedSearch) || email.includes(normalizedSearch) || specialization.includes(normalizedSearch);
    
    const matchesApproval = approvalFilter === 'all' || doctor.approval_status === approvalFilter;
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Link to={`/doctors/${doctor.doctor_id}`}>
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                            doctor.approval_status === 'approved' ? 'bg-green-100 text-green-800' :
                            doctor.approval_status === 'rejected' ? 'bg-red-100 text-red-800' :
